...
def cenario_com_funcionario(api_url):
    ...
    gestor_data = {"email": f"gestor.l4.{random.randint(1000,9999)}@empresa.com",
    ...
    empresa_data = {
        ...
        "nome": "Empresa L4",
        "email": f"contato{random.randint(1000,9999)}@l4.com",
    ...
    filial_a_data = {
        ...
        "nome": "Filial A", "email": f"contato{random.randint(1000,9999)}@fa.com",
        "telefone": f"1192222{random.randint(1000,9999)}",
        "senha": "senha-filial-a",
        "endereco": {
            "cep": "02020000",
            "estado": "SP",
            "cidade": "SP",
    ...
    filial_b_data = {
        ...
        "nome": "Filial B", "email": f"contato{random.randint(1000,9999)}@fb.com",
        "telefone": f"1193333{random.randint(1000,9999)}",
        "senha": "senha-filial-b",
        "endereco": {
            "cep": "03030000",
            "estado": "SP",
            "cidade": "SP",
    ...
    funcionario_data = {
        ...
        "email": f"func.l4.{random.randint(1000,9999)}@empresa.com",
        "nome": "Funcionario L4",
        "senha": "senhaDoFuncionario",
        "tipo_usuario": "FUNCIONARIO",
        "filial_associada": id_filial_a,
        "senha_da_filial": "senha-filial-a"
    }
...
def test_funcionario_acessa_recursos_permitidos(api_url, cenario_com_funcionario):
    ...
    url = (
        f"{api_url}/empresas/"
        f"{setup['id_empresa']}/filiais/{setup['id_filial_correta']}/patrimonios/"
        )
...
def test_funcionario_cria_veiculo_com_sucesso(api_url, cenario_com_funcionario):
    ...
    url = (
        f"{api_url}/empresas/"
        f"{setup['id_empresa']}/filiais/{setup['id_filial_correta']}/veiculos/"
        )
    headers = {'Authorization': f'Bearer {setup["token_funcionario"]}'}
    veiculo_data = {"nome": "Veículo de Teste do Funcionário",
                    "valor": "75000.00",
                    "quantidade": 1,
                    "cor": "Prata",
                    "modelo": "Onix",
...
def test_funcionario_cria_utilitario_com_sucesso(api_url, cenario_com_funcionario):
    ...
    url = (
        f"{api_url}/empresas/"
        f"{setup['id_empresa']}/filiais/{setup['id_filial_correta']}/utilitarios/"
        )
    headers = {'Authorization': f'Bearer {setup["token_funcionario"]}'}
    utilitario_data = {"nome": "Notebook Dell i7",
                       "valor": "5500.00", "quantidade": 10,
                       "descricao": "Notebook para equipe de vendas",
                       "funcao": "Trabalho"
                       }
...
def test_funcionario_cria_imobiliario_com_sucesso(api_url, cenario_com_funcionario):
    ...
    url = (
        f"{api_url}/empresas/"
        f"{setup['id_empresa']}/filiais/{setup['id_filial_correta']}/imobiliarios/"
    )
    headers = {'Authorization': f'Bearer {setup["token_funcionario"]}'}
    imobiliario_data = {"nome": "Sala Comercial 101",
                        "valor": "250000.00", "area": "45.50",
                        "tipo": "Escritório",
                        "endereco": {
                            "cep": "04543000",
                            "estado": "SP",
...
def test_funcionario_nao_acessa_outra_filial(api_url, cenario_com_funcionario):
    ...
    url = (
        f"{api_url}/empresas/"
        f"{setup['id_empresa']}/filiais/{setup['id_filial_errada']}/patrimonios/"
        )
    headers = {'Authorization': f'Bearer {setup["token_funcionario"]}'}
    response = requests.get(url, headers=headers)
...
def test_funcionario_busca_patrimonio(api_url, cenario_com_funcionario):
    ...
    assert create_response.status_code == 201, (
        "Setup do teste 4.4 falhou: não conseguiu criar o veículo para a busca."
    )
    url_busca = f"{url_criacao}?search={nome_unico_veiculo}"
    response = requests.get(url_busca, headers=headers)
    assert response.status_code == 200, (
        f"A busca falhou com status {response.status_code}."
        f"Resposta: {response.text}"
        )

    response_data = response.json()
    assert len(response_data) == 1, "A busca retornou mais ou menos de 1 resultado."
    assert response_data[0]['nome'] == nome_unico_veiculo, (
        "O item encontrado na busca não é o correto."
    )
...
def test_funcionario_lista_patrimonios_da_filial(api_url, cenario_com_funcionario):
    ...
    veiculo_data = {"nome": "Veiculo da Lista",
                    "valor": "50000.00",
                    "quantidade": 1,
                    "cor": "Branco",
                    "modelo": "Strada",
                    "fabricante": "Fiat"}
    response_veiculo = requests.post(f"{base_url}/veiculos/",
                                     headers=headers,
                                     json=veiculo_data
                                     )
    assert response_veiculo.status_code == 201, (
        f"Setup falhou ao criar Veículo: {response_veiculo.text}"
    )

    utilitario_data = {"nome": "Utilitario da Lista",
                       "valor": "1200.00", "quantidade": 5,
                       "descricao": "Cadeira de escritório",
                       "funcao": "Mobiliário"}
    response_utilitario = requests.post(f"{base_url}/utilitarios/",
                                        headers=headers,
                                        json=utilitario_data
                                        )
    assert response_utilitario.status_code == 201, (
        f"Setup falhou ao criar Utilitário: {response_utilitario.text}"
        )

    imobiliario_data = {"nome": "Imobiliario da Lista",
                        "valor": "300000.00",
                        "area": "50.0",
                        "tipo": "Sala",
                        "endereco": {
                            ...
                            "numero": "100"}}
    response_imobiliario = requests.post(f"{base_url}/imobiliarios/",
                                         headers=headers,
                                         json=imobiliario_data)
    assert response_imobiliario.status_code == 201, (
        f"Setup falhou ao criar Imobiliário: {response_imobiliario.text}"
    )
    ...
    assert response.status_code == 200, (
        f"Erro ao listar patrimônios consolidados. "
        f"Resposta: {response.text}"
    )
