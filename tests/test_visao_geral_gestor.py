...
def cenario_gestor_com_empresa_populada(api_url):
    ...
    empresa_data = {
        ...
        "email": f"contato{random_id_empresa}@l5.com", "telefone": f"119777{random_id_empresa}", 
        "senha": "senha-empresa",
        "endereco": {"cep": "05050000", "estado": "SP", "cidade": "SP", "bairro": "Perdizes", "logradouro": "Rua A", "numero": "1"}
    }
    ...
    veiculo_a_data = {"nome": "Carro de Serviço", "valor": "80000.00", "quantidade": 1, "cor": "Branco", "modelo": "Fiorino", "fabricante": "Fiat"}
    r_veiculo_a = requests.post(f"{api_url}/empresas/{id_empresa}/filiais/{id_filial_a}/veiculos/", headers=headers_gestor, json=veiculo_a_data)
    assert r_veiculo_a.status_code == 201, f"Setup falhou: não criou Veículo na Filial A. Resposta: {r_veiculo_a.text}"

    utilitario_a_data = {"nome": "Mesa de Escritório", "valor": "750.00", "quantidade": 5, "descricao": "Mesa de trabalho", "funcao": "Mobiliário"}
    ...
    utilitario_b_data = {"nome": "Notebook Dell", "valor": "5000.00", "quantidade": 10, "descricao": "Notebook i7 para desenvolvimento", "funcao": "Computador"}
