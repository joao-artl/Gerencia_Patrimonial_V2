...
def gestor_autenticado(api_url):
    ...
    assert create_response.status_code == 201, f"Setup falhou: não criou gestor. Resposta: {create_response.text}"
...
def empresa_criada_pelo_gestor(api_url, gestor_autenticado):
    ...
    empresa_data = {
        ...
        "endereco": { "cep": "01001000", "estado": "SP", "cidade": "São Paulo", "bairro": "Sé", "logradouro": "Praça da Sé", "numero": "1" }
    }
    response = requests.post(url, headers=headers, json=empresa_data)
    assert response.status_code == 201, f"A fixture 'empresa_criada_pelo_gestor' falhou. Resposta: {response.text}"
...
def test_gestor_cria_empresa_com_sucesso(empresa_criada_pelo_gestor):
    """Garante que um gestor autenticado pode criar uma nova empresa (validado atráves da Fixture)"""
...
def test_gestor_cria_empresa_sem_endereco_falha(api_url, gestor_autenticado):
    ...
    empresa_data_incompleta = {
        ...
        "email": "contato@incompleta.com", "telefone": "11944443333", "senha": "senha-incompleta"
    }
...
def test_gestor_cria_filial_com_sucesso(api_url, empresa_criada_pelo_gestor):
    ...
    filial_data = {
        ...
        "endereco": { "cep": "02020020", "estado": "SP", "cidade": "São Paulo", "bairro": "Santana", "logradouro": "Rua Voluntários", "numero": "2000" }
    }
