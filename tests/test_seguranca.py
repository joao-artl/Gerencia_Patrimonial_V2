...
def cenario_com_funcionario_logado(api_url):
    ...
    gestor_data = {"email": f"gestor.l6.{random.randint(1000,9999)}@empresa.com", "senha": "123", "cpf": f"111{random.randint(10000,99999)}", "nome": "G", "tipo_usuario": "GESTOR"}
    ...
    empresa_data = { "cnpj": f"111{random.randint(10000000,99999999)}", "nome": "Empresa L6", "email": f"contato{random.randint(1000,9999)}@l6.com", "telefone": f"1191111{random.randint(1000,9999)}", "senha": "senha-empresa", "endereco": {"cep": "1", "estado": "a", "cidade": "b", "bairro": "c", "logradouro": "d", "numero": "e"} }
    ...
    filial_data = { "cnpj": f"222{random.randint(10000000,99999999)}", "nome": "Filial Segura", "senha": "senha-filial", "email": f"contato{random.randint(1000,9999)}@fs.com", "telefone": f"1192222{random.randint(1000,9999)}", "endereco": {"cep": "2", "estado": "b", "cidade": "c", "bairro": "d", "logradouro": "e", "numero": "f"} }
...
def cenario_duas_empresas(api_url):
    ...
    gestor_a_data = {"email": f"gestor.a.{random.randint(1000,9999)}@empresa.com", "senha": "123", "cpf": f"333{random.randint(10000,99999)}", "nome": "GA", "tipo_usuario": "GESTOR"}
    requests.post(f"{api_url}/usuarios/", json=gestor_a_data)
    login_a_response = requests.post(f"{api_url}/token/", json={"email": gestor_a_data['email'], "senha": gestor_a_data['senha']})
    token_gestor_a = login_a_response.json()['access']
    headers_gestor_a = {'Authorization': f'Bearer {token_gestor_a}'}
    empresa_a_data = { "cnpj": f"333{random.randint(10000000,99999999)}", "nome": "Empresa A", "email": f"contato{random.randint(1000,9999)}@a.com", "telefone": f"1193333{random.randint(1000,9999)}", "senha": "s", "endereco": {"cep":"3","estado":"c","cidade":"d","bairro":"e","logradouro":"f","numero":"g"} }
    requests.post(f"{api_url}/empresas/", headers=headers_gestor_a, json=empresa_a_data)

    gestor_b_data = {"email": f"gestor.b.{random.randint(1000,9999)}@empresa.com", "senha": "123", "cpf": f"444{random.randint(10000,99999)}", "nome": "GB", "tipo_usuario": "GESTOR"}
    requests.post(f"{api_url}/usuarios/", json=gestor_b_data)
    login_b_response = requests.post(f"{api_url}/token/", json={"email": gestor_b_data['email'], "senha": gestor_b_data['senha']})
    token_gestor_b = login_b_response.json()['access']
    headers_gestor_b = {'Authorization': f'Bearer {token_gestor_b}'}
    empresa_b_data = { "cnpj": f"444{random.randint(10000000,99999999)}", "nome": "Empresa B", "email": f"contato{random.randint(1000,9999)}@b.com", "telefone": f"1194444{random.randint(1000,9999)}", "senha": "s", "endereco": {"cep":"4","estado":"d","cidade":"e","bairro":"f","logradouro":"g","numero":"h"} }
    empresa_b_response = requests.post(f"{api_url}/empresas/", headers=headers_gestor_b, json=empresa_b_data)
...
def test_funcionario_nao_pode_criar_filial(api_url, cenario_com_funcionario_logado):
    ...
    nova_filial_data = { "cnpj": "666777888000199", "nome": "Filial Ilegal", "senha": "s", "email":"i@i.com", "telefone":"1", "endereco": {"cep":"6","estado":"f","cidade":"g","bairro":"h","logradouro":"i","numero":"j"}}
